...
opts="Mount\nUnmount"
opts=$(echo -e $opts | rofi -dmenu -i -theme themes/mount-material.rasi -mesg "Mount or Unmount Drives" | awk '{print $1}')
[[ "$opts" = "" ]] && echo 'Nothing to selected' && exit

if [ $opts = "Mount" ]; then

    chosen=$(echo "$mountable" | rofi -dmenu -i -theme themes/mount-material.rasi -mesg "Drive to mount" | awk '{print $1}')
    ...
    mountpoint=$(echo "$dirs" | rofi -dmenu -i -theme themes/mount-material.rasi -mesg "Mount point")
    [[ "$mountpoint" = "" ]] && echo 'Nothing selected' && exit

    #mountpoint=/mnt/usb

    if [[ ! -d "$mountpoint" ]]; then
        mkdiryn=$(echo -e "No\nYes" | rofi -dmenu -i -theme themes/mount-material.rasi -mesg "$mountpoint does not exist. Create it?")
        if [[ "$mkdiryn" = Yes ]];then
    ...
    #dunstify "$chosen mounted to $mountpoint"
        ...
        chosen=$(echo "$mounted" | rofi -dmenu -i -theme themes/mount-material.rasi -mesg "Drive to umount" | awk '{print $1}')
    [[ "$chosen" = "" ]] && echo 'Nothing selected' && exit
    #gksu mount "$chosen" && exit
