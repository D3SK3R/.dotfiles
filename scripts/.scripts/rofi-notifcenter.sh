...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme ~/.config/rofi/themes/configNotif.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "Notification Center" -lines 15 -i -no-config)
